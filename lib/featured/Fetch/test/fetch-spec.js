...
describe('Fetch', () => {
    ...
    describe('Body mixin', () => {
        describe('arrayBuffer', () => {
            ...
            it('arrayBuffer handles utf-16le data', () => fetch(`${url}/hello/utf16le`)
                ...
                .then(buf => {
                    expect(buf instanceof ArrayBuffer).toBeTrue();
                    // TODO: xhr under IE9- directly get a responseText rather than `Blob`
        ...
        describe('jsons', () => {
            ...
            it('handles json parse error', () => fetch(`${url}/json-error`)
                .then(response => response.json())
                .catch(error => {
                    expect(() => { throw error; }).toThrowMatching(thrown => {
                        const pattern = ({
                            'chrome'  : 'SyntaxError: Unexpected token o in JSON at position 1',
                            'na'      : 'SyntaxError: Unexpected token o in JSON at position 1', // Chrome Headless
                            'ie'      : /^SyntaxError:/, // ignore i18n message locale
                            // eslint-disable-next-line max-len
                            'firefox' : 'SyntaxError: JSON.parse: unexpected keyword at line 1 column 1 of the JSON data',
                        })[browserName];
                        return pattern.test ? pattern.test(`${thrown}`) : `${thrown}` === pattern;
                    });
                }));
        });
    ...
    describe('fetch method', () => {
        ...
        describe('HTTP methods', () => {
            ...
            // native `fetch()` won't normalize the "patch" method as uppercase, so we have to do it manually

...
describe('Fetch', () => {
    ...
    beforeEach(async () => {
        ...
        !Uint8Array && (Uint8Array = (await require('../../../shim/Uint8Array')).Uint8Array);
        !ArrayBuffer && (ArrayBuffer = (await require('../../../shim/ArrayBuffer')).ArrayBuffer);
        !DataView && (DataView = (await require('../../../shim/DataView')).DataView);
    });
    ...
    describe('Request', () => {
        ...
        IE8_DOM_DEFINE || it('construct with URL instance', () => {
        ...
        describe('formData', () => {
            it('post sets content-type header', () => fetch(`${url}/request`, {
                ...
                .then(json => {
                    ...
                    expect(/^multipart\/form-data;/.test(json.headers['content-type'])).toBe(true);
                }));

            it('formData rejects after body was consumed', () => fetch(`${url}/json`)
                .then(response => {
                    expect(!!response.formData).toBe(true);
                    return response.formData();
                })
                .catch(error => { expect(error instanceof TypeError).toBe(true); }));

            it('parses form encoded response', () => fetch(`${url}/form`)
                .then(response => response.formData())
                .then(form => { expect(form instanceof FormData).toBe(true); }));
        });
    ...
    describe('fetch method', () => {
        ...
        describe('aborting', () => {
            it('Request init creates an AbortSignal without option', () => {
                ...
                expect(!!request.signal).toBe(true);
        ...
        describe('HTTP methods', () => {
            ...
            ['post', 'put', 'PATCH', 'delete'].forEach(method => {
                ...
                it(`supports HTTP ${method.toUpperCase()}`, () => fetch(`${url}/request`, {
                    method,
                    ...hasBody && {body : 'name=Hubot'},
                })
                    .then(response => response.json())
                    .then(request => {
                        expect(request.method).toBe(method.toUpperCase());
                        expect(request.data).toBe(hasBody ? 'name=Hubot' : '');
                    })
                    .catch(error => {
                        expect(() => { throw error; }).toThrow(new Error('Those methods are not supported under IE8'));
                    }));
            });
        });
        ...
        describe('Atomic HTTP redirect handling', () => {
            it('handles 301 redirect response', () => fetch(`${url}/redirect?code=301`)
                .then(response => {
                    expect(response.status).toBe(200);
                    expect(response.ok).toBe(true);
                    expect(/\/hello/.test(response.url)).toBe(true);
                    return response.text();
                })
                .then(body => { expect(body).toBe('hi'); }));

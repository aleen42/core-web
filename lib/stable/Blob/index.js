...
module.exports = new Promise(resolve => resolve((async win => {
    ...
    const {Uint8Array} = await require('../../shim/Uint8Array');
    const {ArrayBuffer} = await require('../../shim/ArrayBuffer');
    const {DataView} = await require('../../shim/DataView');
        ...
        function FakeBlobBuilder() {
            ...
            function Blob(...args) {
                ...
                const data = _.map(ary, chunk => {
                    ...
                    } else if (chunk instanceof ArrayBuffer || isArrayBufferView(chunk)) {

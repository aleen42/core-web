...
describe('Blob', () => {
    ...
    /**
     * PhantomJS throws "TypeError: undefined is not a constructor (evaluating 'URL.createObjectURL(l)')"
     * TODO: polyfill `URL.createObjectURL()` ?
     */
    /PhantomJS/.test(navigator.userAgent) || it('can be converted to an object url', () => {
        const blobURL = URL.createObjectURL(new Blob(['test']));
        expect(/^(?:data|blob):/.test(blobURL)).toBeTrue();
    });

    /PhantomJS/.test(navigator.userAgent) || it('object url has "null" origin', () => {
        const blobURL = URL.createObjectURL(new Blob(['test']));
        expect(blobURL.origin == null).toBeTrue();
    });

    window.fetch && it('object url can be fetched', done => {
        try {
            fetch(URL.createObjectURL(new Blob(['test']))).then(resp => {

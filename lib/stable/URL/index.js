...
module.exports = new Promise(resolve => resolve((async win => {
    ...
    const {DOMException} = await require('../DOMException');
    ...
    (() => {
        try {
            ...
            new URL('file://C:/test.png');
        } catch { return 1; }
    })() && (target.URL = _.overwrite(target.URL, (Fn, context, args) => {
        const [url, base] = [].slice.call(args);

        /**
         * 1. `new URL('file://C:/')` throws error in Chrome under MacOS or Linux
    ...
    // need to patch `URL.createObjectURL(Blob)`
    (!target.URL.createObjectURL && Blob) && patchURLStatic();

    function polyfill() {
        // doesn't do the encoding/decoding dance
        ...
        function URL(url, base) {
            ...
            // NOTE: Not supported under IE8-
            IE8_DOM_DEFINE || Object.defineProperties(self, {
                href         : {
        ...
        target.URL = Object.assign(URL,
            OriginalUrl && _.pickBy(_.objBy(Object.getOwnPropertyNames(OriginalUrl), null, i => {
                try {
    ...
    function patchURLStatic() {
        Object.assign(target.URL, {
            createObjectURL : blob => blob instanceof Blob
                                      && `data:${blob.type};base64,${btoa(String.fromCharCode(...blob._buffer))}`,
        });
    }

...
const {URL, URLSearchParams} = require('./index');
const {browser : {name : browserName, version}, os} = require('detector');
...
describe('URL', () => {
    ...
    it('No need to polyfill / patch', () => {
        expect(window.URL === URL).toBe(
            browserName === 'na'
            || (browserName === 'chrome' && osName !== 'macosx')
            || browserName === 'firefox'
            || (browserName === 'ie' && version === 11)

            // TODO: cannot emulate the patched situations
            || (browserName === 'edge')
        );

        expect(window.URLSearchParams === URLSearchParams).toBe(
            browserName === 'na'
            || browserName === 'chrome'
            || browserName === 'firefox'
            || (browserName === 'ie' && version === 11)
    ...
    it('Parameter Mutation', () => {
        ...
        expect(url.searchParams.has('a')).toBeFalse();
        expect(url.searchParams.has('b')).toBeFalse();
    ...
    it('URLSearchParams', () => {
        ...
        expect(url.searchParams instanceof URLSearchParams).toBeTrue();
...
// Not implemented by the polyfill!
    ...
    it('URL contains native static methods', function () {
            expect(typeof URL.createObjectURL == 'function').toBeTrue();
            expect(typeof URL.revokeObjectURL == 'function').toBeTrue();
    });
    */
    ...
    describe('WPT tests', () => {
        ...
        it('throws with DOMException as argument', () => {
            ...
            expect(() => { new URLSearchParams(DOMException.prototype); })
                ...
                .toThrowError(({
                    ...
                    'na'     : 'Incompatible receiver, DOMException required', // phantomJS
                })[browserName]);
        });
        ...
        it('constructs with sequence of sequences of strings', () => {
            ...
            expect(() => { new URLSearchParams([[1]]); }).toThrowError(({
                ...
                'na'     : 'Expected sequence with length 2', // phantomJS
            })[browserName]);

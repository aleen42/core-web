...
describe('URL', () => {
    let FormData, File, URL, URLSearchParams, DOMException;
    ...
    beforeEach(async () => {
        ...
        !DOMException && (DOMException = (await require('../DOMException')).DOMException);
    });

    it('Need to polyfill / patch', () => {
        expect(window.URL !== URL).toBe(
            // patch
            browserName === 'ie' || (/^(chrome|na)$/.test(browserName) && /^(macosx|linux)$/.test(osName))
        );

        // all launchable browsers to test are no required to polyfill URL and URLSearchParams except IE
        expect(window.URLSearchParams !== URLSearchParams).toBe(browserName === 'ie');
    });
    ...
    describe('WPT tests', () => {
        ...
        it('throws with DOMException as argument', () => {
            ...
            expect(() => { new URLSearchParams(DOMException.prototype); })
                 ...
                 * Firefox throws "TypeError: 'get name' called on an object
                 *                      that does not implement interface DOMException.'"
                 * IE 11 throws "TypeError: Invalid calling object"
                 */
                .toThrowMatching(thrown => ({
                    chrome  : /Illegal invocation/,
                    na      : /Illegal invocation/, // Chrome Headless
                    firefox : /DOMException/,
                    ie      : /Invalid calling object|调用的对象无效/,
                })[browserName].test(`${thrown}`));
        });

...
describe('FormData', () => {
    ...
    it('has()', () => {
        ...
        expect(createFormData(['n1', 'value']).has('n1')).toBe(true);
        expect(createFormData(['n1', 'value']).has('n2')).toBe(false);
        expect(createFormData(['n1', 'value'], ['n2', 'value']).has('n1')).toBe(true);
        expect(createFormData(['n1', 'value'], ['n2', 'value']).has('n2')).toBe(true);
        expect(createFormData(['n3', new Blob(['content'])]).has('n3')).toBe(true);
    });
    ...
    it('disabled()', () => {
        expect(createForm('<input disabled name=foo value=bar>').has('foo')).toBe(false);
        expect(createForm('<fieldset disabled><input name=foo value=bar></fieldset>').has('foo')).toBe(false);
        expect(createForm(`<select multiple name="example">
            <option selected disabled value="foo">Please choose one</option>
        </select>`).has('example')).toBe(false);
    });
    ...
    window.Response && it('upload files', async () => {
        ...
        async function formDataPostFileUploadTest(fileBaseName) {
            ...
            expect(formDataText.startsWith(expectedText)).toBe(true);
        }
